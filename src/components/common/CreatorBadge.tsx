...
export default function CreatorBadge({ username, size = 'md', className = '' }: CreatorBadgeProps) {
  ...
  if (size === 'sm') {
    return (
      <span
        title="Verified App Creator & Lead Architect"
        className={`inline-flex items-center space-x-1 px-2 py-0.5 rounded-full bg-gradient-to-r from-[#D4AF37] via-[#F3E5AB] to-[#05C46B] text-slate-950 font-black text-[9px] uppercase tracking-wider shadow-sm shadow-[#D4AF37]/30 border border-[#D4AF37]/60 font-montserrat animate-pulse ${className}`}
      >
        <Sparkles className="w-2.5 h-2.5 text-slate-900 fill-slate-900" />
        <span>CREATOR</span>
      </span>
    );
  }
  ...
  if (size === 'lg') {
    return (
      ...
      >
        <Sparkles className="w-3.5 h-3.5 text-slate-950 fill-slate-950 animate-bounce" />
